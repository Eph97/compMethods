...
function wait(message="";printwaiting=true,multiple=true)
   if multiple 
      if (length(message) > 0) println(message) end
      if printwaiting print("Waiting...") end
      readline()
   end   
   nothing
end   

function writeio(io,formattuple,varlist...;cr=true,callwait=false,printwaiting=true)
